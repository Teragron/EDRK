# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 21:43:31 2024
...
def RK4(tinc=0.01, tend=4000, alpha=0.05):
    ...
    for i in range(num_steps-1):
        ...
        phi_values[i+1] = phi_values[i] + (k1[0] + 2*k2[0] + 2*k3[0] + k4[0]) / 6
        omega_values[i+1] = omega_values[i] + (k1[1] + 2*k2[1] + 2*k3[1] + k4[1]) / 6 # you should only use w in the second ODE

        E_kin_list[i+1] = (1/6) * m * (l**2) * (omega_values[i+1]**2)
        E_pot_list[i+1] = -m * g * l * np.cos(phi_values[i+1]) / 2
        E_summe_list[i+1] = E_kin_list[i+1] + E_pot_list[i+1]
