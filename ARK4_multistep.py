# -*- coding: utf-8 -*-
"""
Created on Wed Mar 27 05:58:30 2024
...
def ARK4_multistep(tinc=0.01, tend=4000, alpha=0.05, num_updates=3):
    ...
    for i in range(num_steps-1):
        ...
        for j in range(num_updates):  # Perform multiple updates to w
            w -= alpha * delta_w
            

            # Recalculate using the updated w
            phi_dot_next = omega_values[i] + (k1[1] + 2*k2[1] + 2*k3[1] + w*k4[1]) / 6
            E_kin_next = (1/6) * m * (l**2) * (phi_dot_next**2)
            E_pot_next = -m * g * l * np.cos(phi_next) / 2
            E_summe_next = E_kin_next + E_pot_next
            delta_w = (E_summe_next - E_summe_list[i])
            w_values.append(w)  # Append w value to the list
